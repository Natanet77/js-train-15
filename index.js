...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    generatePassword += characters[randomNum];
  }
...
function roundObjectValues(obj) {
  ...
  const roundObject = Object.fromEntries(roundEntries);
  return roundObject;
...
function calculateVolumeCylinder(radius, height) {
  ...
  const calculate = Math.PI * Math.pow(radius, 2) * height;
  const volumeCylinder = Math.ceil(calculate);
...
function getFractionalPart(num) {
  ...
  const fractionalPart = Math.fround(num - Math.trunc(num));
...
function compareAndRound(num1, num2) {
  ...
  const compare = Math.round(Math.max(num1, num2));
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((accum, price) => accum + price);
